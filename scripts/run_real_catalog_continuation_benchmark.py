"""Compare a real-catalog measured-search checkpoint with a cold full-budget run."""
...
from run_real_catalog_six_component_budget import (  # noqa: E402
...
def run(
    ...
    return {
        "schema_version": 1,
        "case": "real 0402HP/GQM18 six-component in-memory continuation checkpoint",
        "input": {
...
def validate(payload: dict) -> None:
    ...
    if failed:
        raise RuntimeError("continuation checkpoint regression: " + ", ".join(failed))
    if not payload["continued"]["checkpoint_reused"]:
        raise RuntimeError("continuation did not reuse the measured-search checkpoint")
    if payload["continued"]["checkpoint_prior_physical_evaluations"] != payload["initial"]["physical_evaluations"]:
        raise RuntimeError("continuation checkpoint lost prior exact-evaluation accounting")
    maximum_balance_error = max(
        payload[name]["maximum_power_balance_error"]
        for name in ("initial", "continued", "cold_total_budget")
    )
    if maximum_balance_error > 1e-9:
        raise RuntimeError(f"power balance regression: {maximum_balance_error:.3g}")
    if comparison["incremental_elapsed_fraction_of_cold"] >= 0.8:
        raise RuntimeError("incremental continuation no longer saves meaningful wall-clock time")
...
def main() -> None:
    ...
    parser.add_argument("--initial-seconds", type=float, default=8.0)
    parser.add_argument("--additional-seconds", type=float, default=12.0)
    parser.add_argument(
        "--output", type=Path,
        default=ROOT / "artifacts/benchmarks/real-catalog-continuation-checkpoint.json",
    )
