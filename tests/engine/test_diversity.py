...
SNP_PATH = SNP_DIR / '3_antenna_no_dielectric.s3p'
...
def test_diversity():
    ...
    if not SNP_PATH.exists():
        pytest.skip(f"external diversity fixture is not installed: {SNP_PATH}")
    ...
    if not DB_PATH.exists():
        pytest.skip(f"external Murata database is not installed: {DB_PATH}")
