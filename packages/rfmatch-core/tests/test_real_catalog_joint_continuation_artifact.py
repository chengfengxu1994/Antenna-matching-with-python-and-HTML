...
ARTIFACT = ROOT / "artifacts/benchmarks/real-catalog-joint-continuation-checkpoint.json"


def test_real_catalog_joint_checkpoint_matches_completed_cold_full_matrix_search():
    payload = json.loads(ARTIFACT.read_text(encoding="utf-8"))
    assert payload["schema_version"] == 1
    assert payload["input"]["catalog_size"] == {"inductors": 49, "capacitors": 322}
    assert payload["input"]["maximum_components_by_port"] == {"0": 1, "1": 1, "2": 1}
    assert payload["budgets"] == {
        "initial_seconds": 5.0,
        "additional_seconds": 15.0,
        "total_seconds": 20.0,
    }
    ...
    assert payload["continued"]["physical_evaluations"] == 577
