...
Supports Touchstone v1 and v2 formats:
- Frequency units: Hz, kHz, MHz, GHz
- Parameter formats: S, Y, Z, H, G
- Data formats: RI (real-imaginary), MA (magnitude-angle in degrees), DB (dB-angle)
- Complex port impedances for multi-port files
"""
...
def _add_sparams(data: TouchstoneData, values: list, n: int):
    """Add S-parameter values for one frequency point."""
    idx_p = 0
...
def load_touchstone_file(filepath: str) -> TouchstoneData:
