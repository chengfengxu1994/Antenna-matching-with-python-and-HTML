...
function marker(sweep, index, cx, cy, radius, color, key) {
  ...
  return <circle key={key} cx={x} cy={y} r="3.5" fill={color} stroke="var(--bg-inset)" strokeWidth="1.5" />;
}
  ...
  return (
    <div className="engineering-smith">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="基于复数 S11 扫频数据的 Smith Chart">
        ...
        <circle cx={cx} cy={cy} r={radius} fill="var(--bg-inset)" stroke="var(--border-strong)" strokeWidth="1.4" />
        <g clipPath="url(#smith-unit-circle)" fill="none" stroke="var(--border)" strokeWidth="0.8">
        ...
        <g fill="var(--text-secondary)" fontSize="9" textAnchor="middle">
        ...
        {entries.map(([port, sweep]) => {
          ...
          return (
            <g key={port} clipPath="url(#smith-unit-circle)">
              {sweep.raw_real?.length === sweep.frequencies.length && <path d={tracePath(sweep, cx, cy, radius, 'raw')} fill="none" stroke="var(--text-faint)" strokeWidth="1" strokeDasharray="4 4" opacity=".72" />}
        ...
        <g transform="translate(610 72)">
          <text x="0" y="0" fill="var(--text)" fontSize="12" fontWeight="700">轨迹说明</text>
          {entries.map(([port], index) => <g key={port} transform={`translate(0 ${24 + index * 24})`}><line x1="0" y1="0" x2="24" y2="0" stroke={COLORS[Number(port) % COLORS.length]} strokeWidth="3" /><text x="34" y="4" fill="var(--text-secondary)" fontSize="11">端口 {Number(port) + 1} · 匹配后</text></g>)}
          <g transform={`translate(0 ${34 + entries.length * 24})`}><line x1="0" y1="0" x2="24" y2="0" stroke="var(--text-faint)" strokeDasharray="4 4" /><text x="34" y="4" fill="var(--text-faint)" fontSize="10">匹配前 DUT</text></g>
          <g transform={`translate(0 ${65 + entries.length * 24})`} fill="var(--text-secondary)" fontSize="10"><circle cx="3" cy="0" r="3" fill="var(--accent)" /><text x="14" y="4">起点 {startGHz.toFixed(3)} GHz</text><circle cx="3" cy="22" r="3" fill="var(--accent)" /><text x="14" y="26">终点 {stopGHz.toFixed(3)} GHz</text></g>
          <text x="0" y={125 + entries.length * 24} fill="var(--text-faint)" fontSize="9">轨迹直接使用扫频 API 返回的</text>
          <text x="0" y={139 + entries.length * 24} fill="var(--text-faint)" fontSize="9">S11 实部与虚部，不做幅度近似。</text>
        </g>
      </svg>
    </div>
  );
