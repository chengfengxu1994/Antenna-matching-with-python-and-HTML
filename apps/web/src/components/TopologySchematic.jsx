...
function componentLabel(component, reference) {
  return `${reference}: ${formatValue(component)}`;
}

function componentDesignators(components) {
  const counts = { L: 0, C: 0 };
  return new Map(components.map(component => {
    ...
    counts[prefix] += 1;
    return [component, `${prefix}${counts[prefix]}`];
  }));
}
    ...
    <div className="schematic-wrap">
      <svg className="schematic-svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Matching topology">
        ...
        {entries.map(([piStr, pm], row) => {
          ...
          const designators = componentDesignators(comps);
              ...
              {series.map((c, ci) => {
                ...
                return (
                  <g key={`s-${ci}`} className="schematic-component">
                    ...
                    <text x={x} y={y - 38} textAnchor="middle" fontSize="18">{componentLabel(c, designators.get(c))}</text>
              ...
              {shunts.map((c, ci) => {
                ...
                return (
                  <g key={`p-${ci}`} className="schematic-component">
                    ...
                    <text x={x + (leftSide ? 26 : -26)} y={y + 46} textAnchor={leftSide ? 'start' : 'end'} fontSize="18">{componentLabel(c, designators.get(c))}</text>
