...
class MurataDatabase:
    ...
    def get_component_sparams(self, component_id, freq_mhz):
        ...
        # Runtime API calls execute in worker threads. A SQLite cursor owns its
        # active result set, so sharing ``self.cursor`` lets an unrelated query
        # replace these rows between execute() and fetchall().
        c = self.conn.cursor()
...
if __name__ == '__main__':
    ...
    murata_dir = str(MURATA_DIR)
    optenni_dir = os.environ.get(
        'RFMATCH_OPTENNI_COMPONENT_DIR',
        str(Path.home() / 'AppData' / 'Roaming' / 'Optenni' / 'ComponentLibrary'),
    )
    db_path = str(MURATA_DIR / 'full_components.db')
    ...
    if mode == 'murata':
        db_path = str(MURATA_DIR / 'murata_components.db')
    elif mode == 'optenni':
        db_path = str(MURATA_DIR / 'optenni_components.db')
    else:
        db_path = str(MURATA_DIR / 'full_components.db')
