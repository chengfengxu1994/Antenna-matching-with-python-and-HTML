...
from .component_environment import ComponentEnvironmentCatalog
...
class MurataDBLibrary:
    ...
    def __init__(self, db_path: str, environment_catalog: ComponentEnvironmentCatalog = None):
    ...
    def environment_coverage(self) -> Dict:
        if self.environment_catalog is None:
            return {"matched_components": 0, "unmatched_records": 0}
        cursor = self.db.conn.cursor()
        known = {
            str(row[0]).strip().casefold()
            for row in cursor.execute("SELECT part_number FROM components")
        }
        requested = set(self.environment_catalog.records)
        return {
            "matched_components": len(known & requested),
            "unmatched_records": len(requested - known),
        }
    ...
    def _record_to_info(self, record: ComponentRecord):
        """Wrap query results with the same S-parameter interface as cached primaries."""
        return DBComponentAdapter(record, self)
...
class DBComponentAdapter:
    ...
    def __init__(self, record, db_library):
        ...
        self.series = record.series
        self.size_code = record.size_code or ""
        self.tolerance_code = record.tolerance_code or ""
        self.tolerance_pct = record.tolerance_pct
        self.voltage_code = record.voltage_code or ""
        self.dielectric = record.dielectric or ""
        # The component row is authoritative for package/tolerance. Manufacturer
        # is stored on the joined series row and resolved once per library.
        if not hasattr(db_library, "_manufacturer_by_series"):
            try:
                db_library._manufacturer_by_series = {
                    item.get("name", ""): item.get("manufacturer", "")
                    for item in db_library.get_series_summary()
                }
            except Exception:
                # Older repository databases predate the manufacturer column.
                # Keep it unknown instead of inventing authoritative metadata.
                db_library._manufacturer_by_series = {}
        self.manufacturer = db_library._manufacturer_by_series.get(record.series, "")
        self.metadata_provenance = {
            "manufacturer": "database" if self.manufacturer else "unknown",
            "series": "database",
            "package_code": "database" if self.size_code else "unknown",
            "tolerance_pct": "database" if self.tolerance_pct is not None else "unknown",
            "voltage_code": "part_number_inferred" if self.voltage_code else "unknown",
            "dielectric": "part_number_inferred" if self.dielectric else "unknown",
        }
        self.tempco_ppm_per_c = None
        self.systematic_bias_pct = None
        self.environment_metadata = {}
        environment = (
            db_library.environment_catalog.lookup(self.part_number)
            if db_library.environment_catalog is not None else None
        )
        if environment is not None:
            self.tempco_ppm_per_c = environment.tempco_ppm_per_c
            self.systematic_bias_pct = environment.systematic_bias_pct
            self.environment_metadata = environment.as_dict()
            label = environment.provenance_label()
            if self.tempco_ppm_per_c is not None:
                self.metadata_provenance["tempco_ppm_per_c"] = label
            if self.systematic_bias_pct is not None:
                self.metadata_provenance["systematic_bias_pct"] = label
...
def load_murata_db(
    db_path: str = None,
    environment_catalog: ComponentEnvironmentCatalog = None,
) -> MurataDBLibrary:
    """
    ...
    if db_path is None:
        ...
        candidates = [str(MURATA_DIR / 'murata_components.db')]
    ...
    return MurataDBLibrary(db_path, environment_catalog=environment_catalog)
