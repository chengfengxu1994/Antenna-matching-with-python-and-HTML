...
RUN pip install --no-cache-dir "fastapi>=0.100.0" "uvicorn>=0.23.0" \
    "pydantic>=2.0.0" "python-multipart>=0.0.6"
RUN pip install --no-cache-dir "reportlab>=3.6.0"
...
RUN useradd --system --uid 10001 --create-home rfmatch \
    && mkdir -p /var/lib/rfmatch/projects /var/lib/rfmatch/snp \
